...
public class SmemConfigurator {
	...
	public File configure(File configFile, File agentDir) throws IOException {
		...
		if(configFile != null && configFile.exists()){
			...
			parseSmemConfigurationFile(configFile, agentDir, smemDir, createdFiles);
	...
	private void parseSmemConfigurationFile(File configFile, File agentDir, File smemDir, Map<String, File> createdFiles) {
		...
		while((line = configReader.readLine()) != null){
			...
			try {
				...
				if(line.startsWith("template") && args.length > 1){
					...
					writeTemplateFile(template, configReader, outputFile, createdFiles);
				}
				...
				// config-file <filename>
					...
					File subConfigFile = new File(agentDir, "/" + args[1]);
					parseSmemConfigurationFile(subConfigFile, agentDir, smemDir, createdFiles);
				}
		...
		// Write all files tagged with include-file <filename>
	...
	/* Look for files sourced by language-comprehension/smem-words/smem-words_to_process.soar
	 * and copy them to the smem folder, mapping any LTI's */
	...
	private void writeTemplateFile(SmemTemplate template, BufferedReader configReader, File outputFile, Map<String, File> createdFiles){
	...
	// re-mapping LTI's to their integer versions and 
	// filtering blocks based on include/exclude lists
	// (A block is a commented line of the form '### +@BLOCK_NAME' and goes until a closing brace)
